  ...
  return (
    <>
      <section className="rounded-[2rem] bg-zinc-950 px-6 py-8 text-white shadow-xl sm:px-8">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            <p className="text-sm font-semibold uppercase tracking-[0.22em] text-sky-300">Quote Follow-Up OS</p>
            <h1 className="mt-3 text-4xl font-semibold tracking-tight sm:text-5xl">Stop letting quotes go cold.</h1>
            <p className="mt-4 max-w-2xl text-base leading-7 text-zinc-300">
              A lean quote-chasing system for small service businesses. Track every quote, see who needs a follow-up today,
              and keep the pipeline moving without buying a bloated CRM.
            </p>
          </div>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="Total quotes" value={String(metrics.totalQuotes)} subtext="All live and closed records in the workspace" />
        <MetricCard label="Value at risk" value={formatCurrency(metrics.valueAtRisk)} subtext="Open quotes that still need an outcome" />
        <MetricCard label="Won revenue" value={formatCurrency(metrics.wonRevenue)} subtext="Quotes already converted into booked work" />
        <MetricCard label="Overdue follow-ups" value={String(metrics.overdueCount)} subtext="Quotes that should have been chased already" />
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.35fr_0.85fr]">
        ...
        <aside className="space-y-4">
          <div>
            ...
            <p className="text-sm text-zinc-500">Ready-to-send nudges for quotes that need action now.</p>
          </div>
          {chaseList.length ? (
